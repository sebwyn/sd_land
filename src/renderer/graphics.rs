...
use uuid::Uuid;
use wgpu::{
    util::DeviceExt, Adapter, BindGroup, BindGroupLayout, Buffer, CommandBuffer, Device, Instance,
    Queue, RenderPipeline, Surface, SurfaceConfiguration, SurfaceError, SurfaceTexture,
};
...
impl Graphics {
    ...
    pub(super) fn begin_render(
        &mut self, /*, clear_color: Option<[f32; 3]>*/
    ) -> Result<(), SurfaceError> {
    ...
    pub(super) fn render(&mut self, work: Vec<GraphicsWork>) -> Result<(), SurfaceError> {
        let view = self
            .current_surface_texture
            .as_ref()
...

        {
            ...
            for task in work.iter() {
                ...
                render_pass
                    .set_index_buffer(task.index_buffer.slice(..), wgpu::IndexFormat::Uint32);
                ...
                render_pass.draw_indexed(
                    0..task.num_indices,
                    0,
                    0..task.num_instances.unwrap_or(1),
                );
            }
        }
    ...
    pub(super) fn resize(&mut self, new_size: PhysicalSize<u32>) {
        if new_size.width > 0 && new_size.height > 0 {
            ...
            self.depth_texture =
                Self::create_depth_texture(&self.device, &self.config, "Some depth texture");
        }
    }
}
...
impl Graphics {
    pub(super) fn create_texture<P, S>(
        &self,
        image: &ImageBuffer<P, S>,
    ) -> Result<wgpu::Texture, SimpleError>
    where
    ...
    pub(super) fn create_sampler(&self, filter_mode: wgpu::FilterMode) -> wgpu::Sampler {
    ...
    pub(super) fn create_bind_groups(
        ...
        for group_index in 0.. {
            let bind_group_layout = bind_group_layouts
                .iter()
                .find(|(index, _)| group_index == *index);
            ...
            for (name, (group, binding), value) in uniforms {
                ...
                let entry = if let Some((_, size)) = groups
                    .iter()
                    .find(|(groups_binding, _)| *groups_binding == binding)
                {
                    ...
                    match value {
                        MaterialValue::Texture(texture) => {
                            let uuid = &texture.uuid.ok_or(SimpleError::new(&format!(
                                "Material was never assigned texture at: {}",
                                name
                            )))?;

                            let texture_view = textures.get(uuid).ok_or(SimpleError::new(
                                &format!("Cannot find texture assigned to material at: {}", name),
                            ))?;
                        ...
                        MaterialValue::Sampler(sampler) => {
                            let sampler = samplers
                                .get(&sampler.uuid.ok_or(SimpleError::new(&format!(
                                    "Material was never assigned sampler: {}",
                                    name
                                )))?)
                                .ok_or(SimpleError::new(&format!(
                                    "Cannot find sampler assigned to material at: {}",
                                    name
                                )))?;
    ...
    pub(super) fn load_pipeline(&mut self, pipeline: Pipeline) -> LoadedPipeline {
        ...
        for group in material_bind_groups {
            ...
            group_and_bind_group_layouts.push((
                group_index,
                self.device
        ...
        let bind_group_layouts = group_and_bind_group_layouts
            .iter()
            .map(|(_, group)| group)
            .collect::<Vec<_>>();
        ...
        LoadedPipeline {
            ...
            bind_group_layouts: group_and_bind_group_layouts,
        }
    }
